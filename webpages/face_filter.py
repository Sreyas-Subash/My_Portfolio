...
from webpages.twilio_turn import get_ice_servers
import av
...
st.sidebar.subheader("Face Filter Controls")
...
if input_option == "Webcam":
    ...
    if uploaded_file is not None:
        ...
        col1, col2, col3 = st.columns([1, 2.5, 1])  # Centering the image
        with col2:
    ...
    imgCol = np.array(Image.open("assets/2.jpeg"))
    imgLips = faceProcess(imgCol, input_option)
    col1, col2, col3 = st.columns([1, 2.5, 1])  # Centering the image
    with col2:
        st.image(imgLips, caption="Processed Image", use_container_width=True)